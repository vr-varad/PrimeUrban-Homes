...
const UpdateListing = () => {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      setFormData(res.data.listings[0]);
    };
  ...
  const handleImageSubmit = (e) => {
    ...
    if (files?.length > 0 && files?.length + formData.images?.length < 7) {
      ...
      Promise.all(promises) 
        ...
        .catch((err) => {
          if(err){setImageUploadError("Image upload failed. (2 mb max per image)");}
          setUploading(false);
        });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`/api/listing/update/${params.listingId}`,dataToSend, {
        headers: {
